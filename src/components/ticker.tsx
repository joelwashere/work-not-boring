...
export const Ticker: FC = () => {
  ...
  //const { data, error } = useSWR("/api/get-eth-data", fetcher, { refreshInterval: 15000 });
  const data = {price: 1};

  const [open, setOpen] = useState(false);
  const usdValue = useRef<HTMLInputElement>(null);
  const [convertedEthValue, setConvertedEthValue] = useState(0);
  ...
  function usdToEth() {
    ...
    if(Number(usdValue.current?.value) !== NaN) {
      setConvertedEthValue(Number((newValue / data?.price).toFixed(3)));
    }
  }
      ...
      <div>
        ...
        {open &&
          <div className="flex gap-1 text-sm justify-between mx-2">
            <label htmlFor="usdv" className="border-none rounded bg-clip-text text-center text-white" placeholder={convertedEthValue.toString()}>
              {convertedEthValue}
            </label>
            <input id="usdv" min={1} ref={usdValue} className="border-solid border-white border rounded bg-clip-text text-center text-white"
              size={7} placeholder="USD here!" onInput={() => {usdToEth()}}/>
          </div>
        }
      </div>
    </div>
